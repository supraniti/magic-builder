      ...
      methods:{
            ...
            window.setTimeout(function () {
              if ( this.dndState === 'DragStartBuffer' ){
                ...
                if (draggable && nestable){
                  ...
                  this.intervalID2 = window.setInterval(this.shadowFix, 20)
        ...
        getValidContainer(){
          ...
          while (el && el !== this.$el && el !== document.body){
            if (el.__vue__){
              let node = el.__vue__
              while (node && node.$children[0] && node.$children[0].$el == el){
                node = node.$children[0]
              }
              let container = this.getContainer(node)
              if (container && this.validate(this.dataObjectComponent,container,this)){
                return container
              }else{
                el = el.parentElement
              }
    ...
    Vue.component('DndContainer',{
      ...
      methods:{
        ...
        getRectangleMap(rectangles){
          ...
          let pointers = []
          let i = 0
          let verticalStructure = true
          while (rectangles[i] && rectangles[i+1]){
            if (rectangles[i].bottom > rectangles[i+1].top){
              verticalStructure = false
            }
            i++
          }
          // if (this.verticalSearch){
          if (verticalStructure || this.verticalSearch){
        ...
        getIndex(x,y){
          ...
          if (this.verticalSearch || rectangleMap.verticalStructure){
        ...
        saveRects(skip){
          ...
          this.nr.rects = els
            ...
            .filter(child=>{
              return child && els.indexOf(child.$el) > -1
            })
